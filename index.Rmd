...
time_tracker <- time_tracker_raw %>%
  ...
  filter(category != "wellness" & category != "exercise") %>%
...
daily_data <- tibble(date = seq(
  ...
  mutate(
    ...
    hrs < light ~ "got fuck all done",
    hrs > light & hrs < moderate ~ "light",
    hrs > moderate & hrs < hardcore ~ "moderate",
    hrs > moderate & hrs < hardcore ~ "hardcore",
...
century %>% 
  ...
  mutate(workload_achieved = fct_relevel(workload_achieved, "eaglescream", "hardcore", "moderate", "light")) %>% 
